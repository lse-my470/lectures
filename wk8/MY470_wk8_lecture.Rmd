...
- Install any IDE (Integrated Development Environment) you prefer afterwards:
  - RStudio
  - VS Code
  - Positron
...
- The classic IDE for R, makes everything easier
...
- Combines the flexibility of VS Code with the R-specific features of RStudio
...
- Unlike in Python, most objects in R are copy-on-modify (with some important exceptions), so can be called immutable in that sense

- It can pay off to study these topics for performance of code regardless of the language. 

- A very good summary for R, which is also the basis of this slide and the next two, can be found here: https://adv-r.hadley.nz/names-values.html
...
x <- c(3, 6, 9)
x[2] <- 4
pryr::address(x)   # check memory address of x
y <- x
...
- Note: Statistical models can require variables be stored as factors
...
A "programming paradigm" refers to a standard practice or perspective on programming (how you **think about** or **approach a problem**).
...
R is at its core a functional programming language:

- Functions are **first-class objects** and can be:
  - Passed to other functions as parameters
  - Returned by other functions as output
  - Stored in data structures (functions are **first class* objects)
- Functions are (should be) **pure**:
...
knitr::include_graphics("figs/functional_programming.png")
```
...
- Python also tends to be more procedural (modifying the global state or mutable objects)
...
```{r, eval = FALSE}
ggplot2::ggplot(my_data, aes(x = x, y = y, color = as.factor(g))) +
  geom_point(size = 0.5) +
  labs(
    x = "X variable",
    y = "Y variable",
...
```{r, out.width = "40%", fig.align="center", echo = FALSE}
knitr::include_graphics("figs/ggplot2_plot_formatted.png")
```

## Grammar of Graphics $\leftrightarrow$ `{ggplot2}` {.smaller}

- A visualisation is a **mapping** of **data** to **aesthetics** via visual **geometries** to produce meaning. 

- Formally this can be represented as a "**grammar of graphics**" (Wilkinson, 2005). 

- In `{ggplot2}`, three key pieces:
  - **Data**: The information
  - **Mapping**: Correspondence between data and aesthetics  (`aes()`)
  - **Layers**: Geometry and transformation (e.g. `geom_point()`, `geom_smooth()`)
...
- MY470 is a course about programming, so we only covered fundamentals of the R language in this lecture
